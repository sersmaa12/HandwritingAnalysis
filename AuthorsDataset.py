...
class AuthorsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 👉 ALWAYS convert both images to grayscale 2D
        if img1.ndim == 3:
            img1 = transforms.functional.to_pil_image(img1)
            img1 = transforms.functional.to_grayscale(img1, num_output_channels=1)
            img1 = np.array(img1)

        if img2.ndim == 3:
            img2 = transforms.functional.to_pil_image(img2)
            img2 = transforms.functional.to_grayscale(img2, num_output_channels=1)
            img2 = np.array(img2)
...
class Downsample(object):
    ...
    def __call__(self, sample):
        img1, img2 = sample
        img1 = transform.rescale(img1, self.scale_factor, channel_axis=None)
        img2 = transform.rescale(img2, self.scale_factor, channel_axis=None)
        return (img1, img2)
...
def scale(image, scale):
    return transform.rescale(image, scale, channel_axis=None)
